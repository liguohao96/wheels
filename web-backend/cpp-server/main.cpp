...
int main(int argc, char* argv[]) {
    ...
    std::string data;
    std::function<void(const boost::system::error_code&, std::shared_ptr<HttpConnection>)> accept_handler;
    auto router = std::shared_ptr<router_type>(new router_type());
    (*router)["/"]["GET"] = [](std::shared_ptr<Request> req, std::shared_ptr<Response> res) {
    ...
    (*router)["/index.html"]["GET"] = [](std::shared_ptr<Request> req, std::shared_ptr<Response> res) {
        auto& response = res->data;
        response << "HTTP/1.1 200 OK\r\n";
        response << "Content-Type: text/html\r\n";
        std::string response_data = "<!DOCTYPE html><html><head></head><body><h1>hello world!</h1></body></html>";
    ...
    accept_handler = [&io_ctx, &connections, &tcp_acceptor, &accept_handler, &router](const boost::system::error_code& ec, std::shared_ptr<HttpConnection> conn) {
        ...
        if (!ec) {
            ...
            // }
            // boost::asio::async_write(tcp_socket, boost::asio::buffer(data), write_handler);
            // std::shared_ptr<HttpConnection> conn = std::make_shared<HttpConnection>(sock_ptr);
            // connections.push_back(conn);
            conn->deal();
        }
        // std::unique_ptr<ip::tcp::socket> new_sock_ptr = std::make_unique<ip::tcp::socket>(io_ctx);
        std::unique_ptr<ip::tcp::socket> new_sock_ptr(new ip::tcp::socket(io_ctx));
        std::shared_ptr<HttpConnection> new_conn = std::make_shared<HttpConnection>(std::move(new_sock_ptr), router);
        tcp_acceptor.async_accept(*new_conn->_socket, std::bind(accept_handler, std::placeholders::_1, new_conn));
    };
    // std::unique_ptr<ip::tcp::socket> tcp_socket = std::make_unique<ip::tcp::socket>(io_ctx);
    std::unique_ptr<ip::tcp::socket> tcp_socket(new ip::tcp::socket(io_ctx));
    std::shared_ptr<HttpConnection> new_conn = std::make_shared<HttpConnection>(std::move(tcp_socket), router);
